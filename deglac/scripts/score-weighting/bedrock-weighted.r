...
#nc = nc_open(file)
 ...
 # }
  ...
  pd=length(time)
  nc_close(nc)
  nom[i,]=ss.norm[i]*zb.ngrip
  print(i)
}

mean.zb.ngrip=c()
for(t in 1:length(time)){
  mean.zb.ngrip[t]=sum(nom[,t],na.rm=T)/sum(ss.norm,na.rm=T)
}
file.out=paste(work.fldr,"/scripts/score-weighting/mean-zb-ngrip.txt",sep="")
write.table(mean.zb.ngrip, file.out, quote = F, sep = " ", row.names=F)
...
for(i in 1:nr.best){
  ...
  pd=length(time)
  nc_close(nc)
  nom[i,]=ss.norm[i]*(zb.ngrip-mean.zb.ngrip)^2
  print(i)
}
...
for(t in 1:length(time)){
  sd.zb.ngrip[t]=sqrt(sum(nom[,t],na.rm=T)/(w*sum(ss.norm,na.rm=T)))
}
file.out=paste(work.fldr,"/scripts/score-weighting/sd-zb-ngrip.txt",sep="")
write.table(sd.zb.ngrip, file.out, quote = F, sep = " ", row.names=F)
