...
#nc = nc_open(file)
  ...
  pd=length(time)
  nc_close(nc)
  nom[i,]=ss.norm[i]*smb.ngrip
  print(i)
}

mean.smb.ngrip=c()
for(t in 1:length(time)){
  mean.smb.ngrip[t]=sum(nom[,t],na.rm=T)/sum(ss.norm,na.rm=T)
}
file.out=paste(work.fldr,"/scripts/score-weighting/mean-smb-ngrip.txt",sep="")
write.table(mean.smb.ngrip, file.out, quote = F, sep = " ", row.names=F)
...
for(i in 1:nr.best){
  ...
  pd=length(time)
  nc_close(nc)
  nom[i,]=ss.norm[i]*(smb.ngrip-mean.smb.ngrip)^2
  print(i)
}
...
for(t in 1:length(time)){
  sd.smb.ngrip[t]=sqrt(sum(nom[,t],na.rm=T)/(w*sum(ss.norm,na.rm=T)))
}
file.out=paste(work.fldr,"/scripts/score-weighting/sd-smb-ngrip.txt",sep="")
write.table(sd.smb.ngrip, file.out, quote = F, sep = " ", row.names=F)
